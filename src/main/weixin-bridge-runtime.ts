...
function readWeixinPackageInfo(): WeixinPackageInfo {
  ...
  if (!packageJson) {
    throw new Error(
      'Built-in WeChat login component is missing. Reinstall Claude360 Copilot or rebuild with @tencent-weixin/openclaw-weixin bundled.'
    )
  }
    ...
    switch (recordString(status, 'status')) {
      ...
      case 'binded_redirect':
        ...
        return {
          ...
          sessionKey,
          message: '已连接过此 Claude360 Copilot，无需重复连接。'
        }
      ...
      case 'confirmed': {
        ...
        return {
          ...
          baseUrl,
          userId,
          message: '已将此 Claude360 Copilot 连接到微信。'
        }
      }
    }
    await sleep(1_000)
...
async function postToDeepSeekGuiWebhook(message: WeixinMessage, accountId: string): Promise<JsonRecord> {
  ...
  if (!res.ok || data.ok === false) {
    throw new Error(recordString(data, 'message') || `Claude360 Copilot webhook HTTP ${res.status}`)
  }
  return data
}
