 ...
 *   仍落在 workspace assets 目录内（防目录穿越）。
 */
...
export class MediaAssetsService {
  ...
  private async resolveAssetAbsolutePath(workspaceRoot: string, relativePath: string): Promise<string> {
  ...
  private enqueueWrite<R>(workspace: string, task: () => Promise<R>): Promise<R> {
    ...
    this.writeChains.set(workspace, next)
    return next
  }
  ...
  async readAssetBlob(payload: MediaAssetsReadBlobPayload): Promise<MediaAssetsReadBlobResult> {
    try {
      const absolute = await this.resolveAssetAbsolutePath(payload.workspaceRoot, payload.relativePath)
