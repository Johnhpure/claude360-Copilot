  ...
  return (
    <div className="ds-no-drag flex min-h-0 flex-1 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-0.5 pb-1">
        {channels.length === 0 ? (
          <div className="mx-2 mt-2 rounded-2xl border border-dashed border-ds-border-muted bg-[color-mix(in_srgb,var(--ds-bg-main)_35%,transparent)] px-3 py-4">
...
/**
 * 第三方 IM 品牌标识（微信 / Telegram / 飞书官方 logo 配色）。
 * 品牌色属内容/领域色，不随主题反转、不映射 UI token（逐行 token-exempt 标记）。
 */
export function ClawProviderLogo({
  ...
  if (provider === 'telegram') {
    return (
      <span
        className={`inline-flex items-center justify-center rounded-full bg-[#27A7E7] text-white ${className}`} // token-exempt: Telegram brand blue
        aria-hidden="true"
      >
        <svg
        ...
        >
          ...
          <path
            ...
            fill="#DFF4FF" // token-exempt: Telegram brand light blue
          />
        </svg>
      </span>
    )
  ...
  return (
    ...
    >
      <path
        ...
        fill="#00D6B9" // token-exempt: Feishu brand teal
      />
      <path
        ...
        fill="#3370FF" // token-exempt: Feishu brand blue
      />
      <path
        ...
        fill="#133C9A" // token-exempt: Feishu brand navy
      />
    </svg>
  )
}
