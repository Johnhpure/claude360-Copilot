...
export function WriteImagePreview({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[image:radial-gradient(circle_at_top,color-mix(in_srgb,var(--ds-accent)_10%,transparent),transparent_34%),linear-gradient(180deg,var(--ds-bg-canvas),var(--ds-bg-main))]">
      ...
      <div className="min-h-0 flex-1 overflow-auto p-4 sm:p-6">
        <div className="flex min-h-full items-center justify-center">
          <img
            src={src}
            alt={fileName}
            className={`${actualMode ? 'max-w-none' : 'max-h-full max-w-full'} select-none rounded-lg object-contain shadow-[var(--c360-shadow-sm)]`}
